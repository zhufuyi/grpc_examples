...
func OpenTracingClientInterceptor(tracer opentracing.Tracer, optFuncs ...Option) grpc.UnaryClientInterceptor {
	...
	return func(ctx context.Context, method string, req, resp interface{}, cc *grpc.ClientConn, invoker grpc.UnaryInvoker, opts ...grpc.CallOption) error {
		...
		//		tracer = ginTracer.(opentracing.Tracer)
		//	}
		//	if parentSpan, ok := ginContext.Get("parentSpan"); ok {
		...
		if ginTracer, ok := ctx.Value(GinTracerKey).(opentracing.Tracer); ok {
			tracer = ginTracer
		}
		if ginParentSpan, ok := ctx.Value(GinParentSpanKey).(*jaegerClient.Span); ok {
			parentCtx = ginParentSpan.Context()
		} else {
			if parent := opentracing.SpanFromContext(ctx); parent != nil {
				parentCtx = parent.Context()
			}
		...
		clientSpan := tracer.StartSpan(method+" [client]", opentracing.ChildOf(parentCtx), ext.SpanKindRPCClient, gRPCComponentTag)
