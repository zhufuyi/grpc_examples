...
protoc --gogofast_out=${outPath} --gogofast_opt=paths=source_relative --go-grpc_out=${outPath} --go-grpc_opt=paths=source_relative *.proto

# 生成gin.pb.go文件，http的api接口文件
protoc --go-gin_out=${outPath} --go-gin_opt=paths=source_relative *.proto
